...
class Acados_NMPC_Nominal:
    def __init__(self,params_dict):
        ...
        self.horizon = self.params['horizon']  # Define the number of discretization steps
        self.dt = self.params['dt']
        self.T_horizon = self.horizon * self.dt
        self.use_RTI = self.params['use_RTI']
        self.use_integrators = self.params['use_integrators']
        self.use_warm_start = self.params['use_warm_start']
        self.use_foothold_constraints = self.params['use_foothold_constraints']

        self.use_static_stability = self.params['use_static_stability']
        self.use_zmp_stability = self.params['use_zmp_stability']
        self.use_stability_constraints = self.use_static_stability or self.use_zmp_stability

        self.use_DDP = self.params['use_DDP']
    ...
    def create_ocp_solver_description(self, acados_model) -> AcadosOcp:
        ...
        if (self.use_DDP):
            ...
            ocp.solver_options.nlp_solver_max_iter = self.params['num_qp_iterations']
            ...
            if (self.params['as_rti_type'] == "AS-RTI-A"):
                ocp.solver_options.as_rti_iter = 1
                ocp.solver_options.as_rti_level = 0
            elif (self.params['as_rti_type'] == "AS-RTI-B"):
                ocp.solver_options.as_rti_iter = 1
                ocp.solver_options.as_rti_level = 1
            elif (self.params['as_rti_type'] == "AS-RTI-C"):
                ocp.solver_options.as_rti_iter = 1
                ocp.solver_options.as_rti_level = 2
            elif (self.params['as_rti_type'] == "AS-RTI-D"):
                ocp.solver_options.as_rti_iter = 1
        ...
        if (self.params['solver_mode'] == "balance"):
            ocp.solver_options.hpipm_mode = "BALANCE"
        elif (self.params['solver_mode'] == "robust"):
            ocp.solver_options.hpipm_mode = "ROBUST"
        elif (self.params['solver_mode'] == "fast"):
            ocp.solver_options.qp_solver_iter_max = 10
            ocp.solver_options.hpipm_mode = "SPEED"
        elif (self.params['solver_mode'] == "crazy_speed"):
        ...
        if (self.params['use_nonuniform_discretization']):
            time_steps_fine_grained = np.tile(self.params['dt_fine_grained'],
                                              self.params['horizon_fine_grained'])
            time_steps = np.concatenate(
                (time_steps_fine_grained, np.tile(self.dt, self.horizon - self.params['horizon_fine_grained'])))
    ...
    def create_friction_cone_constraints(self, ) -> None:
        ...
        f_max = self.params['grf_max']
        f_min = self.params['grf_min']
            ...
            for j in range(0, self.horizon):
                ...
                if (self.use_stability_constraints):
                    ...
                    if (FL_contact_sequence[j] == 1 and
                        ...
                        # TROT
                        stability_margin = self.params['trot_stability_margin']
                        ...
                        # PACE
                        stability_margin = self.params['pace_stability_margin']
                        if (FL_contact_sequence[j] == 1 and FR_contact_sequence[j] == 0):
                        ...
                        stability_margin = self.params['crawl_stability_margin']
    ...
    def compute_control(self, state, reference, contact_sequence, constraint=None, external_wrenches=np.zeros((6,)),
        ...
        for j in range(self.horizon):
            ...
            if (self.params['external_wrenches_compensation'] and
                    self.params['external_wrenches_compensation_num_step'] and
                    j < self.params['external_wrenches_compensation_num_step']):
        ...
        if (self.use_integrators):
            ...
            alpha_integrator = self.params["alpha_integrator"]
            ...
            cap_integrator_z = self.params["integrator_cap"][0]
            cap_integrator_x_dot = self.params["integrator_cap"][1]
            cap_integrator_y_dot = self.params["integrator_cap"][2]
            cap_integrator_z_dot = self.params["integrator_cap"][3]
            cap_integrator_roll = self.params["integrator_cap"][4]
            cap_integrator_pitch = self.params["integrator_cap"][5]
        ...
        if (self.params['dt'] <= 0.02 or (
                self.params['use_nonuniform_discretization'] and self.params['dt_fine_grained'] <= 0.02)):
